...
def test_cli_entrypoint_preserves_checkout_edge_case_contract(tmp_path):
    ...
    events = {str(event["UID"]): event for event in _events(out_path.read_text(encoding="utf-8"))}
    assert set(events) == {"one-night-cli@google.com", "timed-cli@google.com"}
    assert events["one-night-cli@google.com"]["DTSTART"].to_ical() == b"20260515"
    assert events["one-night-cli@google.com"]["DTEND"].to_ical() == b"20260516"
    assert events["timed-cli@google.com"]["DTSTART"].to_ical() == b"20260601T160000"
    assert events["timed-cli@google.com"]["DTEND"].to_ical() == b"20260602T100000"
...
def test_whitelisted_properties_only_in_events():
    ...
    for ev in _events(out):
        for key in ev.keys():
            ...
            assert key in KEEP_IN_EVENT, f"non-whitelisted property in VEVENT: {key}"
...
def test_shift_dtend_standalone_helper():
    ...
    assert shift_dtend_back_one_day("DTEND;VALUE=DATE:20260430") == "DTEND;VALUE=DATE:20260429"
    assert (
        shift_dtend_back_one_day("DTEND:20260301") == "DTEND:20260228"
    )  # Feb/March boundary, non-leap year
    assert shift_dtend_back_one_day("DTEND:20240301") == "DTEND:20240229"  # leap year
    ...
    assert shift_dtend_back_one_day("DTEND:20260602T100000Z") == "DTEND:20260602T100000Z"
    # Non-DTEND lines untouched
    assert shift_dtend_back_one_day("DTSTART;VALUE=DATE:20260430") == "DTSTART;VALUE=DATE:20260430"
