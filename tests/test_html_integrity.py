...
def test_no_placeholder_patterns(html_text: str):
    for pattern, reason in PLACEHOLDER_PATTERNS:
        ...
        assert not matches, (
            f'placeholder survived in index.html: {pattern!r} '
            f'({reason}); occurrences={matches[:3]}'
        )
...
def test_canonical_link_matches_cname(soup: BeautifulSoup,
                                      cname_domain: str):
    link = soup.find('link', rel='canonical')
    assert link is not None, 'no <link rel=canonical> found'
    href = link.get('href', '')
    assert cname_domain in href, (
        f'<link rel=canonical href={href!r}> does not reference '
        f'CNAME domain {cname_domain!r}'
    )
...
def test_og_url_matches_cname(soup: BeautifulSoup, cname_domain: str):
    ...
    assert cname_domain in content, (
        f'og:url={content!r} does not reference CNAME domain '
        f'{cname_domain!r}'
    )
...
def test_every_en_element_has_pl_counterpart_in_same_parent(
    ...
    for parent_id, counts in parents_with_lang.items():
        ...
        if en != pl:
            ...
            parent = next(
                (el.parent for el in soup.find_all(attrs={'data-lang': True})
                 if id(el.parent) == parent_id),
                None,
            )
            tag_name = getattr(parent, 'name', '?')
            offenders.append(
                f'<{tag_name}> has en={en} pl={pl} data-lang children'
            )
...
_ICAL_CONST_RE = re.compile(
    r"const\s+ICAL_(CLIFFS|GARDENS)\s*=\s*'([^']*)'"
)
...
def test_ical_constants_not_default_placeholders(html_text: str):
    ...
    assert len(matches) == 2, (
        f'expected two ICAL_ constants, got {len(matches)}: {matches}'
    )
    for name, value in matches:
        assert value != f'{name}_ICAL_URL', (
            f'ICAL_{name} still set to default placeholder'
        )
        # Must point to an https URL or a local same-origin path.
        assert value.startswith(('https://', 'ical/', '/ical/')), (
            f'ICAL_{name} = {value!r} does not look like a URL or '
            f'same-origin path'
        )
